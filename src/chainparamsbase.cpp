...
void SetupChainParamsBaseOptions()
{
    ...
    gArgs.AddArg("-posactivationheight=<n>", "Set proof-of-stake activation height (regtest-only)", true, OptionsCategory::CHAINPARAMS);
...
std::unique_ptr<CBaseChainParams> CreateBaseChainParams(const std::string& chain)
{
    if (chain == CBaseChainParams::MAIN)
        return MakeUnique<CBaseChainParams>("", 33603);
    else if (chain == CBaseChainParams::TESTNET)
        return MakeUnique<CBaseChainParams>("testnet4", 43603);
