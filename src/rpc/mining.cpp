        ...
        throw std::runtime_error(
            RPCHelpMan{"generatetoaddress",
                "\nMine blocks immediately to a specified address (before the RPC call returns)\n",
                {
                    ...
                    {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The address to send the newly generated deltacoin to."},
            ...
            + "If you are running the deltacoin core wallet, you can get a new address to send the newly generated deltacoin to with:\n"
...
static UniValue getstakinginfo(const JSONRPCRequest& request)
{
    ...
    const uint64_t nNetworkWeight = GetPoSNetworkWeight();
...
static UniValue getblocktemplate(const JSONRPCRequest& request)
{
    ...
    if (g_connman->GetNodeCount(CConnman::CONNECTIONS_ALL) == 0)
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "Deltacoin is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "Deltacoin is downloading blocks...");
