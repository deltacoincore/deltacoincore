...
- [Development guidelines](#development-guidelines)
    - [General Deltacoin Core](#general-deltacoin-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
Deltacoin Core uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
Run with the `-testnet` option to run with "play deltacoins" on the test network, if you
...
Deltacoin Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/deltacoin-project/deltacoin/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_deltacoin
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_deltacoin --log_level=test_suite
$ valgrind -v --leak-check=full src/deltacoind -printtoconsole
```
...
# A coverage report will now be accessible at `./test_deltacoin.coverage/index.html`.
```
...
To profile a running deltacoind process for 60 seconds, you could use an
invocation of `perf record` like this:

```sh
$ perf record \
    -g --call-graph dwarf --per-thread -F 140 \
    -p `pgrep deltacoind` -- sleep 60
```
...
Deltacoin Core can be compiled with various "sanitizers" enabled, which add
...
- ThreadSocketHandler : Sends/Receives data from peers on port 33604.
...
- ThreadRPCServer : Remote procedure call handler, listens on port 33603 for connections and services them.
...
However, in open source software such as Deltacoin Core, where everyone uses
...
pay attention to for reviewers of Deltacoin Core code.

General Deltacoin Core
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: Deltacoin Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
Some of these are maintained by active developers of Deltacoin Core, in which case changes should probably go
directly upstream without being PRed directly against the project.  They will be merged back in the next
subtree merge.

Others are external projects without a tight relationship with our project.  Changes to these should also
be sent upstream but bugfixes may also be prudent to PR against Deltacoin Core so that they can be integrated
...
file descriptors it will cause problems with Deltacoin's `select()` loop, because
...
would be to revert the upstream fix before applying the updates to Deltacoin's
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `deltacoin-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `deltacoin-cli` and the GUI debug console use this table to determine how to
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of Deltacoin Core, the wallet was always
